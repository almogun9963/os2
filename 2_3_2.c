...
void test(int phnum)
{
    if(state[phnum] == HUNGRY && (fork_arr[LEFT] == EATING || fork_arr[RIGHT] == EATING))
    {
        checkingPhnum = phnum + 1;
        if (fork_arr[RIGHT] == EATING)
            fork_arr[RIGHT] = checkingPhnum;
        else if (fork_arr[LEFT] == EATING)
...
void put_fork(int phnum)
{
    ...
    fork_arr[LEFT+1] = EATING;
    fork_arr[phnum+1] = EATING;
